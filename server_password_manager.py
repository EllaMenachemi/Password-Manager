...
def main():
    ...
    while data != "EXIT":
        ...
        if data[:17] == "username sign up:":
            check_username_sign_up(client_socket, data)
        if data[:17] == "password sign up:":
            check_password_sign_up(client_socket, data)
        if data[:16] == "username log in:":
            check_username_and_password_log_in(client_socket, data)
        if data[:15] == "ADD check site:":
            add_username_to_new_website(client_socket, data)
        if data[:19] == "ADD check password:":
            add_password(client_socket, data)
        if data[:11] == "check site:":
            check_if_site_exists(client_socket, data)
        if data[:22] == "UPDATE check password:":
            add_password(client_socket, data)
        length = client_socket.recv(2).decode()
        data = client_socket.recv(int(length)).decode()

    exit_program(server_socket, client_socket)


def check_if_site_exists(client_socket, data):
    data = "check site:facebook"
    site = data[11:]
    url = site
    extracted_info = tldextract.extract(url)
    site = extracted_info.domain + ".txt"
    file_path = "C:\\Users\\Ella Menachemi\\OneDrive\\Desktop\\password_manager\\zvika1\\" + site
    ...
    if not file.exists():
        ...
        path = ("C:\\Users\\Ella Menachemi\\OneDrive\\Desktop\\password_manager\\zvika1\\" + site)
        file = open(path, "r")
    ...
    send_client_message(client_socket, reply)


def check_password_validity(client_socket, password):
    is_valid = True
    small = False
    capital = False
    if len(password) < 8:
...
def send_client_message(client_socket, reply):
...
def check_username_sign_up(client_socket, data):
    data = data[16:]
    file_path = "C:\\Users\\Ella Menachemi\\OneDrive\\Desktop\\password_manager\\" + "zvika1"
    ...
    if file.exists():
        ...
        path = os.path.join(parent_dir, "zvika1")
        os.mkdir(path)
        print("sign up 2")
        send_client_message(client_socket, reply)


def check_password_sign_up(client_socket, data):
    data = "password sign up:Em123456789(verify:)Em123456789"
    j = 0
    while j != len(data):
        if data[j] == "(":
            break
    ...
    verify = "Em123456789"
    reply, is_valid = check_password_validity(client_socket, password)
    ...
    if is_valid:
        ...
        path = "C:\\Users\\Ella Menachemi\\OneDrive\\Desktop\\password_manager\\zvika1"
...
def check_username_and_password_log_in(client_socket, data):
...
def add_username_to_new_website(client_socket, data):
    ...
    if not is_url:
        try:
            ...
            path = "C:\\Users\\Ella Menachemi\\OneDrive\\Desktop\\password_manager\\zvika1\\" + site_name
        ...
        try:
            ...
            print('Yeah !  found ')
            path_with_file_name = ("C:\\Users\\Ella Menachemi\\OneDrive\\Desktop\\password_manager\\zvika1\\"
                                   + site_name + ".txt")
            file = pathlib.Path(site_name)
...
def add_password(client_socket, data):
    ...
    if data[:3] == "ADD":
        ...
        while j != len(data):
            if data[j] == ")":
                break
            j += 1
        site_password = data[22:j]
        site = data[j+1:]
        print("password: " + site_password)
        print("site: " + site)
    reply, is_valid = check_password_validity(client_socket, site_password)
    if is_valid:
        if add:
            path = "C:\\Users\\Ella Menachemi\\OneDrive\\Desktop\\password_manager\\zvika1\\*"
...
def exit_program(server_socket, client_socket):
    client_socket.close()
    server_socket.close()
